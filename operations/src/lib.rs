...
pub struct PrependNote {
    pub note: Note,
    pub duration: Beat,
    pub selections: Option<Vec<u32>>,
}

impl Operation for PrependNote {
    fn apply(&self, ctx: &mut Context) {
        ...
        for selection_id in selections {
            ...
            for (idx, event) in ctx.score.events.iter().enumerate() {
                if event.start > selection.begin.0 {
                    break
                }
                target_idx = 0;
            }
            let mut insertion_beat = selection.begin.0;
            for event in &mut ctx.score.events {
                if event.start >= insertion_beat {
                    event.start += self.duration;
                }
            }

            for selection in &mut ctx.selections.0 {
                if selection.begin.0 > insertion_beat {
                    selection.begin.0 += self.duration;
                }
                if selection.end.0 >= insertion_beat {
                    selection.end.0 += self.duration;
                }
            }

            ctx.score.events.insert(target_idx, Event {
                note: self.note,
                start: insertion_beat,
